...
const TFGM_KEY = {"Ocp-Apim-Subscription-Key": "352d46528de74094835792b951c110fc"};
...
class Home extends Component {
	constructor(props) {
		...
		this.handleGoBackToCurrentLocation = this.handleGoBackToCurrentLocation.bind(this);
	}
	...
	* */
	...
	handleGetNearByBusStop(latitude, longitude) {
		fetch(`http://transportapi.com/v3/uk/bus/stops/near.json?${USERKEY1}&lat=${latitude}&lon=${longitude}`)
			.then((res) => res.json())
			.then((res) => {
				this.setState({
	...
	handleGetNearByTrainStop(latitude, longitude) {
		fetch(`http://transportapi.com/v3/uk/train/stations/near.json?${USERKEY1}&lat=${latitude}&lon=${longitude}`)
			.then((res) => res.json())
			.then((res) => {
				this.setState({
	...
	handleGetCurrentLocation() {
		navigator.geolocation.getCurrentPosition((pos => {
			...
			this.handleGetNearByBusStop(this.state.currentLocationMe.latitude, this.state.currentLocationMe.longitude);
			this.handleGetNearByTrainStop(this.state.currentLocationMe.latitude, this.state.currentLocationMe.longitude);
			...
			<View style={{flex: 1}}>
				...
				<View style={styles.container}>
					<MapView
						...
						onRegionChangeComplete={e => console.log("onRegionChangeComplete:", e.latitude, e.longitude)}
						showsCompass={true}
						showsTraffic={true}
						scrollEnabled={true}
						showsUserLocation={true}
						onUserLocationChange={e => console.log("onUserLocationChange:", e.latitude, e.longitude)}
					...
					>
							...
							this.state.currentLocationTrain.map((trainStop) => {
								return (
									<Train
										...
										coordinate={{latitude: trainStop.latitude, longitude: trainStop.longitude}}
									/>
								);
							})
						}
						{
							...
							this.state.currentLocationCarPark.map((carpark) => {
								let info = carpark.Location.LocationSpatial.Geography.WellKnownText;
								let info_latitude = parseFloat(info.substring(7, info.length - 1).split(" ")[1]);
								let info_longitude = parseFloat(info.substring(7, info.length - 1).split(" ")[0]);
								return (
									<CarPark
										description={carpark.Description}
										key={carpark.Id}
										id={carpark.Id}
										coordinate={{latitude: info_latitude, longitude: info_longitude}}
									/>
								);
							})
						}
